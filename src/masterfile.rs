...
///
#[pyfunction]
pub fn create_masterfile(path: String, password: String) -> PyResult<()> {
    ...
    let mut key = argon2::hash_raw(password.as_bytes(), &master_salt, &argon2_config).unwrap();
    ...
    if !path.ends_with('/') {
        let mut masterfile = fs::File::create(format!("{}/masterfile.e", path))?;

        // Write master salt and nonce unencrypted
        // for later decryption
        masterfile.write_all(&master_salt)?;
        masterfile.write_all(&master_nonce)?;
        masterfile.write_all(
            &aead
                .encrypt(&nonce_ga, master_key.as_ref())
                .expect("Failure")[..],
        )?;
        masterfile.write_all(
            &aead
                .encrypt(&nonce_ga, folder_salt.as_ref())
                .expect("Failure")[..],
        )?;
        masterfile.write_all(
            &aead
                .encrypt(&nonce_ga, folder_nonce.as_ref())
                .expect("Failure")[..],
        )?;
    } else {
        let mut masterfile = fs::File::create(format!("{}masterfile.e", path))?;
...
pub fn read_masterfile(path: &str, password: &str) -> Result<MasterfileData, anyhow::Error> {
    ...
    // Decrypt data
    let master_key = aead
        .decrypt(&nonce_ga, encrypted_master_key.as_ref())
        .unwrap();
    let folder_salt = aead
        .decrypt(&nonce_ga, encrypted_folder_salt.as_ref())
        .unwrap();
    let folder_nonce = aead
        .decrypt(&nonce_ga, encrypted_folder_nonce.as_ref())
        .unwrap();
